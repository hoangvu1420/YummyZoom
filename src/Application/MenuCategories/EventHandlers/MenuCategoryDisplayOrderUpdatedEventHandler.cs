...
/// <summary>
/// Handles MenuCategoryDisplayOrderUpdated domain events by rebuilding the FullMenuView for the affected restaurant.
/// Strategy: naive, correct-first full rebuild per restaurant.
/// Idempotent via inbox infrastructure.
/// </summary>
public sealed class MenuCategoryDisplayOrderUpdatedEventHandler : IdempotentNotificationHandler<MenuCategoryDisplayOrderUpdated>
{
    private readonly IMenuReadModelRebuilder _rebuilder;
    private readonly IMenuRepository _menuRepository;
    private readonly ILogger<MenuCategoryDisplayOrderUpdatedEventHandler> _logger;

    public MenuCategoryDisplayOrderUpdatedEventHandler(
        IUnitOfWork uow,
        IInboxStore inbox,
        IMenuReadModelRebuilder rebuilder,
        IMenuRepository menuRepository,
        ILogger<MenuCategoryDisplayOrderUpdatedEventHandler> logger)
        : base(uow, inbox)
    {
    ...
    protected override async Task HandleCore(MenuCategoryDisplayOrderUpdated notification, CancellationToken ct)
    {
        _logger.LogInformation("Handling MenuCategoryDisplayOrderUpdated (EventId={EventId}, CategoryId={CategoryId}, MenuId={MenuId})",
        ...
        if (menu is null)
        {
            _logger.LogWarning("Menu with ID {MenuId} not found when handling MenuCategoryDisplayOrderUpdated event", notification.MenuId.Value);
            return;
        }
