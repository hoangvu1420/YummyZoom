...
/// <summary>
/// Handles MenuCategoryNameUpdated domain events by rebuilding the FullMenuView for the affected restaurant.
...
public sealed class MenuCategoryNameUpdatedEventHandler : IdempotentNotificationHandler<MenuCategoryNameUpdated>
{
    ...
    protected override async Task HandleCore(MenuCategoryNameUpdated notification, CancellationToken ct)
    {
        _logger.LogInformation("Handling MenuCategoryNameUpdated (EventId={EventId}, CategoryId={CategoryId}, MenuId={MenuId})",
        ...
        if (menu is null)
        {
            _logger.LogWarning("Menu with ID {MenuId} not found when handling MenuCategoryNameUpdated event", notification.MenuId.Value);
            return;
        }
