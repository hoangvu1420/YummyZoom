...
/// <summary>
/// Handles MenuCategoryRemoved domain events by rebuilding the FullMenuView for the affected restaurant.
/// Strategy: naive, correct-first full rebuild per restaurant.
/// Idempotent via inbox infrastructure.
/// </summary>
public sealed class MenuCategoryRemovedEventHandler : IdempotentNotificationHandler<MenuCategoryRemoved>
{
    private readonly IFullMenuViewMaintainer _rebuilder;
    private readonly IMenuRepository _menuRepository;
    private readonly ILogger<MenuCategoryRemovedEventHandler> _logger;
    ...
    public MenuCategoryRemovedEventHandler(
        ...
        ILogger<MenuCategoryRemovedEventHandler> logger)
        : base(uow, inbox)
    {
    ...
    protected override async Task HandleCore(MenuCategoryRemoved notification, CancellationToken ct)
    {
        ...
        if (menu is null)
        {
            _logger.LogWarning("Menu with ID {MenuId} not found when handling MenuCategoryRemoved event", notification.MenuId.Value);
            return;
        }
