...
public sealed class CustomizationChoiceRemovedEventHandler : CustomizationGroupProjectorBase<CustomizationChoiceRemoved>
{
    public CustomizationChoiceRemovedEventHandler(
        IUnitOfWork uow,
        IInboxStore inbox,
        IMenuReadModelRebuilder rebuilder,
        ICustomizationGroupRepository groupRepository,
        ILogger<CustomizationChoiceRemovedEventHandler> logger)
        : base(uow, inbox, rebuilder, groupRepository, logger)
    { }

    protected override async Task HandleCore(CustomizationChoiceRemoved notification, CancellationToken ct)
    {
