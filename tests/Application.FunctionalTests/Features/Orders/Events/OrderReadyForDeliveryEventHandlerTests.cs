...
public class OrderReadyForDeliveryEventHandlerTests : BaseTestFixture
{
    ...
    public async Task MarkOrderAsReadyForDelivery_Should_Trigger_OrderReadyForDelivery_Broadcast_And_Inbox_Record()
    {
        ...
        notifierMock.Verify(
            n => n.NotifyOrderStatusChanged(It.IsAny<OrderStatusBroadcastDto>(), It.IsAny<CancellationToken>()), Times.AtLeast(3)); // Accepted, Preparing, ReadyForDelivery
    ...
    public async Task DrainingOutbox_Twice_Should_Not_Duplicate_Broadcast()
    {
        ...
        notifierMock.Verify(
            n => n.NotifyOrderStatusChanged(It.IsAny<OrderStatusBroadcastDto>(), It.IsAny<CancellationToken>()), Times.Exactly(3));
