...
public class TeamCartPushNotifierFlowTests : BaseTestFixture
{
    private sealed class TestTeamCartRealtimeNotifier : ITeamCartRealtimeNotifier
    {
        ...
        private Task FanOut(TeamCartId id, CancellationToken ct) => _push.PushTeamCartDataAsync(id, ct);
    ...
    public async Task MemberJoined_Should_Emit_Push()
    {
        ...
        pushMock
            .Setup(p => p.PushTeamCartDataAsync(It.IsAny<TeamCartId>(), It.IsAny<CancellationToken>()))
        ...
        pushMock.Verify(p => p.PushTeamCartDataAsync(It.Is<TeamCartId>(id => id.Value == scenario.TeamCartId), It.IsAny<CancellationToken>()), Times.AtLeastOnce);
    }
