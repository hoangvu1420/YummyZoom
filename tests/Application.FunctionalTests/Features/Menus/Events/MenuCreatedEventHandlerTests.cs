...
/// <summary>
/// Functional tests for <see cref="MenuCreatedEventHandler"/> verifying:
...
/// </summary>
public class MenuCreatedEventHandlerTests : BaseTestFixture
{
    [Test]
    public async Task CreateMenu_ShouldRebuildFullMenuView_AndBeIdempotent()
    {
        // Arrange
        ...
        // Act: send command which emits MenuCreated
        var result = await SendAsync(cmd);
        result.ShouldBeSuccessful();
        var menuId = result.Value!.MenuId;

        // First outbox drain processes MenuCreated and rebuilds the view
        ...
        {
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            var handlerName = typeof(YummyZoom.Application.Menus.EventHandlers.MenuCreatedEventHandler).FullName!;
            ...
            var processedOutbox = await db.Set<OutboxMessage>()
                .Where(m => m.Type.Contains("MenuCreated"))
                .ToListAsync();
            ...
            view.LastRebuiltAt.Should().BeAfter(baselineRebuiltAt, "view should be rebuilt after menu create event");
        }
