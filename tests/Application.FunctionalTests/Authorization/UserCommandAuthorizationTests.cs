...
public class UserCommandAuthorizationTests : BaseTestFixture
{
    ...
    #region User Authorization Service Integration Tests

    [Test]
    public async Task IdentityService_UserAuthorization_ShouldWorkCorrectly()
    {
        // Arrange
        var userId = await RunAsDefaultUserAsync();
        ...
        // Create test commands as resources
        var ownUserCommand = new TestUserOwnerCommand(userId);      // User's own data
        var otherUserCommand = new TestUserOwnerCommand(_testUserId1); // Someone else's data

        // Act - Test authorization for own data
        var ownDataResult = await identityService.AuthorizeAsync(userId.ToString(), "MustBeUserOwner", ownUserCommand);
        
        // Act - Test authorization for other user's data
        var otherDataResult = await identityService.AuthorizeAsync(userId.ToString(), "MustBeUserOwner", otherUserCommand);

        // Assert
        ownDataResult.Should().BeTrue("User should have access to their own data");
        otherDataResult.Should().BeFalse("User should not have access to other user's data");
    }

    [Test]
    public async Task IdentityService_AdminUserAuthorization_ShouldAccessAnyUser()
    {
        // Arrange
        var adminUserId = await RunAsAdministratorAsync();
        ...
        // Create test commands for different users
        var user1Command = new TestUserOwnerCommand(_testUserId1);
        var user2Command = new TestUserOwnerCommand(_testUserId2);

        // Act - Test admin authorization for any user data
        var user1Result = await identityService.AuthorizeAsync(adminUserId.ToString(), "MustBeUserOwner", user1Command);
        var user2Result = await identityService.AuthorizeAsync(adminUserId.ToString(), "MustBeUserOwner", user2Command);

        // Assert
        user1Result.Should().BeTrue("Admin should have access to any user's data");
        user2Result.Should().BeTrue("Admin should have access to any user's data");
    }
