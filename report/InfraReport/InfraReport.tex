...
Để hiện thực hóa kiến trúc Cloud Native và đảm bảo khả năng vận hành ổn định, đồ án tận dụng hệ sinh thái đa dạng các dịch vụ Platform-as-a-Service (PaaS) và Serverless trên Microsoft Azure. Nền tảng của toàn bộ quy trình này bắt đầu từ .NET Aspire, một stack hiện đại đóng vai trò là bộ điều phối (Orchestrator). Aspire cho phép định nghĩa các thành phần hệ thống và mối quan hệ giữa chúng bằng mã nguồn C\#, từ đó đảm bảo tính nhất quán tuyệt đối giữa môi trường phát triển cục bộ và hạ tầng triển khai thực tế trên đám mây.

Các tài nguyên Azure được tổ chức một cách khoa học trong Resource Group (RG) - một thùng chứa logic giúp quản lý vòng đời và kiểm soát chi phí tập trung. Thành phần tính toán cốt lõi là Azure Container Apps (ACA), nền tảng serverless container cho phép vi dịch vụ tự động mở rộng linh hoạt theo lưu lượng mà không cần quản lý hạ tầng Kubernetes phức tạp. Để đảm bảo an ninh mạng, các Container Apps được đặt trong một môi trường an toàn gọi là Azure Container Apps Environment (ACE), tạo ra biên giới mạng ảo riêng biệt cho phép giao tiếp nội bộ thông qua tên dịch vụ mà không phơi bày ra internet.

Việc lưu trữ và quản lý dữ liệu cũng được tối ưu hóa thông qua các chuyên biệt dịch vụ. Azure Container Registry (ACR) đóng vai trò là kho lưu trữ riêng tư, đảm bảo tính an toàn cho các Docker Image trước khi triển khai. Đối với dữ liệu bền vững, Azure Database for PostgreSQL (Flexible Server) cung cấp khả năng quản trị hoàn toàn với độ tin cậy cao. Toàn bộ các thông tin nhạy cảm của hệ thống được bảo vệ tại Azure Key Vault, kết hợp với giải pháp Managed Identity (MI) để các dịch vụ tự xác thực với nhau mà không cần sử dụng mật khẩu tĩnh hay chuỗi kết nối thủ công. Cuối cùng, mọi hoạt động của hạ tầng đều được giám sát tập trung tại Log Analytics Workspace (LAW), nơi thu thập và phân tích nhật ký để phục vụ công tác vận hành và xử lý sự cố.
...
Toàn bộ quy trình trên được điều phối tự động bởi GitHub Actions. Pipeline CI/CD thực hiện song song hai nhiệm vụ: đóng gói và đẩy Container Image lên ACR, đồng thời sử dụng Bicep (thông qua lệnh `azd up`) để cung cấp tài nguyên hạ tầng. Cuối cùng, mọi dữ liệu nhật ký và số liệu giám sát từ môi trường chạy đều được đẩy về Log Analytics Workspace, tạo thành một vòng lặp DevSecOps khép kín và an toàn. Chi tiết về quy trình pipeline sẽ được trình bày trong Chương \ref{chap:devops-cicd}.
