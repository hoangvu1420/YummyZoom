...
Mẫu CQRS giúp hệ thống tách biệt rõ ràng luồng Đọc (Query) và Ghi (Command) dữ liệu. Phần Ghi sử dụng Entity Framework Core kết hợp với Domain Model để đảm bảo tính toàn vẹn dữ liệu khi thực hiện các thay đổi nghiệp vụ. Trong khi đó, Phần Đọc sử dụng Dapper với SQL để truy vấn dữ liệu trực tiếp và trả về các DTO phẳng, tối ưu hóa tốc độ phản hồi cho người dùng.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{6.2cm}|p{3.2cm}|p{5.6cm}|}
        ...
        Số endpoint API & 158 & Tổng số endpoint API được sử dụng bởi frontend \\ \hline
...
\noindent
\begin{minipage}{\linewidth}
Tính năng Đặt hàng nhóm (TeamCart) là điểm nhấn công nghệ của dự án, cho phép nhiều người dùng cùng thao tác trên một giỏ hàng trong thời gian thực. Hình \ref{fig:demo-mobile-teamcart-lobby} thể hiện sự đồng bộ trạng thái giữa màn hình của chủ phòng (Host) và thành viên (Member). Khi bất kỳ ai thêm hoặc bớt món, danh sách món ăn và tổng tiền sẽ được cập nhật tức thì trên tất cả thiết bị nhờ công nghệ SignalR, tạo cảm giác cộng tác mượt mà như đang ngồi cùng nhau.
...
\begin{figure}[H]
    ...
    \caption{Giao diện TeamCart: Màn hình Chủ phòng (Trái) và Thành viên (Phải)}
...
\begin{itemize}
    ...
    \item \textbf{Kỹ thuật kiểm thử sử dụng:} Kiểm thử hộp đen, Kiểm thử tích hợp (xác minh tính đồng bộ thời gian thực qua SignalR).
...
\begin{longtable}{|p{1cm}|p{4.5cm}|p{4cm}|p{5.5cm}|}
    ...
    1 & Tạo mới TeamCart & Nhấn nút "Bắt đầu nhóm" & Giỏ hàng nhóm được tạo, Link mời được sinh ra. Người tạo trở thành Chủ phòng (Host). \\ \hline
...
\subsection*{Triển khai Backend (Azure Container Apps, azd và Bicep)}
