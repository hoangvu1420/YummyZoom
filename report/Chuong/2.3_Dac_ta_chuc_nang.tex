...
\begin{longtable}{|p{2.5cm}|p{1.5cm}|p{3cm}|p{5.5cm}|}
    ...
    \textbf{Luồng sự kiện chính} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
    ...
    \textbf{Luồng sự kiện thay thế} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
...
\begin{longtable}{|p{2.5cm}|p{1.5cm}|p{3cm}|p{5.5cm}|}
    ...
    \textbf{Luồng sự kiện chính} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
     ...
     & 9 & Khách hàng & Chọn phương thức thanh toán (CreditCard, PayPal, ApplePay, GooglePay, \gls{cod}). \\ \cline{2-4} 
     & 10 & Hệ thống & Tạo \gls{paymentintent} (Stripe) nếu thanh toán online, lưu PaymentIntentId vào Order. \\ \cline{2-4} 
     & 11 & Hệ thống & Tạo Order với trạng thái PendingPayment (online) hoặc Placed (\gls{cod}). \\ \cline{2-4} 
    ...
    \textbf{Luồng sự kiện thay thế} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
     ...
     & 10a & Hệ thống & Báo lỗi nếu không tạo được \gls{paymentintent} (lỗi Stripe \gls{api}). \\ \hline
    \textbf{Hậu điều kiện} & \multicolumn{3}{p{11.5cm}|}{Đơn hàng được tạo thành công với trạng thái phù hợp. Sự kiện OrderPlaced được phát ra để thông báo cho nhà hàng. Nếu thanh toán online, khách hàng được chuyển đến trang thanh toán Stripe. Nếu \gls{cod}, đơn hàng chuyển trạng thái Placed ngay lập tức.} \\ \hline
\end{longtable}
...
\begin{longtable}{|c|p{4.0cm}|p{3.0cm}|c|p{3.5cm}|}
    ...
    12 & PaymentMethod & Phương thức thanh toán & Có & CreditCard, PayPal, ApplePay, GooglePay, \gls{cod} \\ \hline
    ...
    16 & TeamCartId & ID giỏ hàng nhóm & Không & GUID hợp lệ (nếu từ \gls{teamcart}) \\ \hline
\end{longtable}
...
\begin{longtable}{|p{2.5cm}|p{1.5cm}|p{3cm}|p{5.5cm}|}
    \hline
    \textbf{Mã use case} & UC-004a & \textbf{Tên use case} & Khởi tạo \gls{teamcart} \\ \hline
    \textbf{Tác nhân} & \multicolumn{3}{p{11.5cm}|}{Khách hàng (Host)} \\ \hline
    \textbf{Mục đích sử dụng} & \multicolumn{3}{p{11.5cm}|}{Cho phép khách hàng tạo một giỏ hàng nhóm (\gls{teamcart}) để mời những người khác cùng đặt món từ một nhà hàng cụ thể.} \\ \hline
    ...
    \caption{Đặc tả Use Case Khởi tạo \gls{teamcart}} \label{tab:uc_004a} \\
    \endlastfoot
    ...
    \textbf{Luồng sự kiện chính} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
     ...
     & 3 & Hệ thống & Tạo mới \gls{teamcart} với trạng thái Active. Tạo ShareToken duy nhất. \\ \cline{2-4} 
     & 4 & Hệ thống & Thêm người tạo vào danh sách thành viên với vai trò Host. \\ \cline{2-4} 
     & 5 & Hệ thống & Trả về thông tin \gls{teamcart} và Link chia sẻ (ShareToken). \\ \hline
    \textbf{Luồng sự kiện thay thế} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
     & 2a & Hệ thống & Báo lỗi nếu nhà hàng không tồn tại hoặc thời gian chốt đơn không hợp lệ (trong quá khứ). \\ \hline
    \textbf{Hậu điều kiện} & \multicolumn{3}{p{11.5cm}|}{\gls{teamcart} được tạo thành công. Host nhận được link chia sẻ để gửi cho các thành viên khác.} \\ \hline
\end{longtable}

Dữ liệu đầu vào cho use case Khởi tạo \gls{teamcart}:
...
\begin{longtable}{|c|p{3cm}|p{4cm}|c|p{3cm}|}
    ...
    \caption{Dữ liệu đầu vào Khởi tạo \gls{teamcart}} \label{tab:input_uc_004a} \\
    \endlastfoot
...
\begin{longtable}{|p{2.5cm}|p{1.5cm}|p{3cm}|p{5.5cm}|}
    \hline
    \textbf{Mã use case} & UC-004b & \textbf{Tên use case} & Tham gia \gls{teamcart} \\ \hline
    ...
    \textbf{Sự kiện kích hoạt} & \multicolumn{3}{p{11.5cm}|}{Khách hàng truy cập vào đường dẫn chia sẻ (Share Link) của \gls{teamcart}.} \\ \hline
    \textbf{Tiền điều kiện} & \multicolumn{3}{p{11.5cm}|}{Khách hàng đã đăng nhập. \gls{teamcart} tồn tại, đang hoạt động (Active) và chưa bị khóa. Mã chia sẻ (ShareToken) hợp lệ.} \\ \hline
    \endfirsthead
    ...
    \caption{Đặc tả Use Case Tham gia \gls{teamcart}} \label{tab:uc_004b} \\
    \endlastfoot

    \textbf{Luồng sự kiện chính} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
     & 1 & Khách hàng & Nhập tên hiển thị (GuestName) để tham gia nhóm. \\ \cline{2-4} 
     & 2 & Hệ thống & Kiểm tra sự tồn tại của \gls{teamcart}. \\ \cline{2-4} 
     & 3 & Hệ thống & Xác thực mã chia sẻ (ShareToken). \\ \cline{2-4} 
     & 4 & Hệ thống & Kiểm tra trạng thái \gls{teamcart} (phải là Active) và quyền tham gia (chưa tham gia trước đó). \\ \cline{2-4} 
     & 5 & Hệ thống & Thêm người dùng vào danh sách thành viên với vai trò Guest. \\ \cline{2-4} 
     & 6 & Hệ thống & Thông báo thành công và chuyển hướng đến giao diện chi tiết \gls{teamcart}. \\ \hline
    \textbf{Luồng sự kiện thay thế} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
     & 2a & Hệ thống & Báo lỗi nếu \gls{teamcart} không tồn tại. \\ \cline{2-4} 
     & 3a & Hệ thống & Báo lỗi nếu mã chia sẻ không hợp lệ hoặc đã hết hạn. \\ \cline{2-4} 
     & 4a & Hệ thống & Báo lỗi nếu \gls{teamcart} đã bị khóa, đã chốt đơn hoặc người dùng đã là thành viên. \\ \hline
    \textbf{Hậu điều kiện} & \multicolumn{3}{p{11.5cm}|}{Người dùng trở thành thành viên của \gls{teamcart}, có quyền thêm món ăn vào giỏ chung.} \\ \hline
\end{longtable}

Dữ liệu đầu vào cho use case Tham gia \gls{teamcart}:
...
\begin{longtable}{|c|p{3cm}|p{4cm}|c|p{3cm}|}
    ...
    \caption{Dữ liệu đầu vào Tham gia \gls{teamcart}} \label{tab:input_uc_004b} \\
    \endlastfoot

    1 & TeamCartId & ID giỏ hàng nhóm & Có & GUID hợp lệ, tồn tại \\ \hline
    2 & ShareToken & Mã chia sẻ & Có & Tối đa 100 ký tự, khớp với \gls{teamcart} \\ \hline
...
\begin{longtable}{|p{2.5cm}|p{1.5cm}|p{3cm}|p{5.5cm}|}
    \hline
    \textbf{Mã use case} & UC-004c & \textbf{Tên use case} & Chốt đơn \gls{teamcart} \\ \hline
    \textbf{Tác nhân} & \multicolumn{3}{p{11.5cm}|}{Khách hàng (Host)} \\ \hline
    \textbf{Mục đích sử dụng} & \multicolumn{3}{p{11.5cm}|}{Cho phép Host khóa giỏ hàng, chờ các thành viên thanh toán và hoàn tất việc chuyển đổi \gls{teamcart} thành đơn hàng chính thức.} \\ \hline
    \textbf{Sự kiện kích hoạt} & \multicolumn{3}{p{11.5cm}|}{Host nhấn nút "Đặt hàng" sau khi đã đủ điều kiện.} \\ \hline
    \textbf{Tiền điều kiện} & \multicolumn{3}{p{11.5cm}|}{\gls{teamcart} đang hoạt động (Active) và có món ăn. Host đã đăng nhập. Để thanh toán, giỏ phải được khóa và đang ở trạng thái Finalized. Để đặt hàng, giỏ phải ở trạng thái ReadyToConfirm.} \\ \hline
    \endfirsthead
    ...
    \caption{Đặc tả Use Case Chốt đơn \gls{teamcart}} \label{tab:uc_004c} \\
    \endlastfoot

    \textbf{Luồng sự kiện chính} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
     & 1 & Host & Chọn chức năng "Khóa đơn" để ngăn chặn việc thêm/bớt món. \\ \cline{2-4} 
     & 2 & Hệ thống & Tính toán chi phí (Tổng tiền, Thuế, Phí vận chuyển), chia tiền cho từng thành viên và chuyển trạng thái sang "Locked". \\ \cline{2-4} 
     & 3 & Host & Áp dụng/điều chỉnh Mã giảm giá hoặc Tip (tùy chọn) khi giỏ đang Locked. \\ \cline{2-4} 
     & 4 & Host & Nhấn "Chốt giá" (Finalize Pricing) để cố định tip/coupon. \\ \cline{2-4} 
     & 5 & Hệ thống & Kiểm tra trạng thái Locked, cố định Tip/Coupon, tăng Quote Version, chuyển trạng thái sang "Finalized" và thông báo thành viên có thể thanh toán. \\ \cline{2-4} 
     & 6 & Thành viên & Chọn phương thức thanh toán (Online hoặc \gls{cod}) và thanh toán/commit phần của mình. \\ \cline{2-4} 
     & 7 & Hệ thống & Ghi nhận thanh toán; nếu chưa đủ thì chờ các thành viên khác. Khi đủ, chuyển trạng thái \gls{teamcart} sang "ReadyToConfirm" và kích hoạt nút "Đặt hàng" cho Host. \\ \cline{2-4} 
     & 8 & Host & Nhập thông tin giao hàng và xác nhận đặt hàng cuối cùng. \\ \cline{2-4} 
     & 9 & Hệ thống & Kiểm tra tính nhất quán (Quote Version, trạng thái ReadyToConfirm, tổng tiền đã đóng). \\ \cline{2-4} 
     & 10 & Hệ thống & Chốt hạn mức Coupon (nếu có), tạo Order và chuyển trạng thái \gls{teamcart} sang "Converted". Thông báo đặt hàng thành công cho tất cả thành viên. \\ \hline
    \textbf{Luồng sự kiện thay thế} & \textbf{STT} & \textbf{Thực hiện bởi} & \textbf{Hành động} \\ \cline{2-4} 
     & 4a & Hệ thống & Từ chối chốt giá nếu \gls{teamcart} không ở trạng thái Locked (CannotFinalizePricingInCurrentStatus). \\ \cline{2-4} 
     & 6a & Hệ thống & Từ chối thanh toán nếu \gls{teamcart} chưa ở trạng thái Finalized (CanOnlyPayOnFinalizedCart). \\ \cline{2-4} 
     & 9a & Hệ thống & Báo lỗi nếu thông tin báo giá (Quote) đã thay đổi hoặc không khớp (xung đột Quote Version). \\ \cline{2-4} 
     & 9b & Hệ thống & Báo lỗi nếu tổng số tiền thanh toán từ các thành viên chưa đủ khớp với tổng đơn hàng. \\ \cline{2-4} 
     & 10a & Hệ thống & Báo lỗi nếu Coupon hết lượt sử dụng tại thời điểm chốt đơn. \\ \hline
    \textbf{Hậu điều kiện} & \multicolumn{3}{p{11.5cm}|}{Đơn hàng được tạo thành công trên hệ thống. \gls{teamcart} hoàn tất vòng đời và được chuyển đổi thành một đơn hàng bình thường, Host có thể theo dõi đơn hàng này.} \\ \hline
\end{longtable}
...
\begin{longtable}{|c|p{3cm}|p{4cm}|c|p{3cm}|}
    ...
    \caption{Dữ liệu đầu vào Chốt giá \gls{teamcart}} \label{tab:input_uc_004c_finalize} \\
    \endlastfoot

    1 & TeamCartId & ID giỏ hàng nhóm & Có & GUID hợp lệ, trạng thái Locked \\ \hline
    2 & QuoteVersion & Phiên bản báo giá hiện tại & Có & Số nguyên không âm, khớp phiên bản mới nhất \\ \hline
\end{longtable}

Dữ liệu đầu vào cho use case Chốt đơn \gls{teamcart} (Bước xác nhận cuối cùng):
...
\begin{longtable}{|c|p{3cm}|p{4cm}|c|p{3cm}|}
    ...
    \caption{Dữ liệu đầu vào Chốt đơn \gls{teamcart}} \label{tab:input_uc_004c} \\
    \endlastfoot
